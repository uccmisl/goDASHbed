...
sudo goDashBed.py -b 10 --videoclients 3 --duration 40 --voipclients 1 --debud="off" --numruns 1 -tm "tcp"
'''
...
urls = full_url_list+main_url_list+live_url_list + full_byte_range_url_list+main_byte_range_url_list
...
def throttleLink(bw_a):
    ...
    for i in range(len(bw_a)):
        ...
        os.system("tc class change dev s1-eth1 parent 1:0 classid 1:1 htb rate %fkbit ceil %fkbit"%(bw_a[i][1],bw_a[i][1]))
        ...
        if num_of_sec > int(args.duration)+5:
            ...
            os.system("tc class change dev s1-eth1 parent 1:0 classid 1:1 htb rate %fkbit ceil %fkbit"%(10000,10000))
            return
        else:            
            sleep(bw_a[i][0]/1000)
...
def monitor_qlen(iface, interval_sec = 0.01, fname='qlen.txt'):
    ...
    while 1:
        ...
        if matches and len(matches) > 1:
            t = "%f" % time()
            open(fname, 'a').write(t + ',' + matches[-2] + ','+ matches_d[-2] + '\n')
            open(fname2, 'a').write(t + ',' + matches[-1] + ','+ matches_d[-1] + '\n')
        sleep(interval_sec)
    return

def qmon(pat):
    monitor = Process(target=monitor_qlen,args=("s1-eth1", 0.01, '%s_sw0-qlen.txt' % pat ))
    monitor.start()
...
def rmon(pat):
    monitor = Process(target=monitor_devs_ng, args=('%s_bytes_sent.txt' % pat, 1.0 ))
    monitor.start()
...
def start_video_clients(num_video, algorithm, buffer_level, server_ip, net, subf, run, **params):
    ...
    for i in range(2,2+int(num_video)):
        ...
        # - files
        log_folder = params["output_folder"] + "/R"+str(run) + params["current_folder"]+log_folder_name+"/"+client_name
        print (log_folder)
        ...
        cmd = params["cwd"]+"/../goDash/DashApp/src/goDASH/goDASH -config " + \
            params["output_folder"]+"/R"+str(run)+params["current_folder"]+config_folder_name+client_config
        print (cmd)
        #CLI(net)
...
def start_voip_clients(server_host, client_host, num, subfolder, run):
    ...
    voip_s1 = client_host.popen("ITGRecv -l %s/recv_%d_log_R%d"%(subfolder, num, run))
...
def prepare_voip_clients(num_voip, host, dur):
    ip_address = host.cmdPrint("ifconfig %s-eth0 | grep inet | awk '{print $2}' | sed 's/addr://'"%host.name).split()[0]
    print ("-----------")
...
def goDashBedNet():
    ...
    prepare_voip_clients(int(args.voipclients), voip_host, 1000*int(args.duration))
    ...
    ip_address_sh = serverHost.cmdPrint("ifconfig %s-eth0 | grep inet | awk '{print $2}' | sed 's/addr://'"%serverHost.name).split()[0]
    ...
    if args.transport_mode == "quic":
        ...
        tt3 = serverHost.cmd("./example '-bind=www.godashbed.org:443' '-www=/var/www/html' &")
    elif args.transport_mode == "tcp":
        print ("Starting TCP server")
        #tt = serverHost.cmd('./caddy -host %s -port 8080 -root /var/www/html &'%ip_address_sh)
        tt = serverHost.cmd('./caddy -conf ./caddy-config/TestbedTCP/Caddyfile &')
    sleep(3)
    #print (tt)
    ...
    print (s1.intfList())
    print (s0.intfList()[2:])
    subfolder = args.scenarioname + "_R"+str(args.numruns) + '/godash_' + "algname" +  '_' + str(total_num_hosts).zfill(3)
    os.system("mkdir -p %s"%subfolder)

    trace_files = ["traces/"+f for f in listdir("traces/") if isfile(join("traces/", f))]  
    ...
    for run in range(1,1+int(args.numruns)):
        for trace_file in trace_files:
            ...
            if args.sched == "fifo":
                print ("fifo")
                getVersion =  subprocess.Popen("bash tc_fifo.sh %s %d"%("s1-eth1",args.bw_net), shell=True, stdout=subprocess.PIPE).stdout
            for intf in s0.intfList()[2:]:
                getVersion2 =  subprocess.Popen("bash tc_delay.sh %s %d"%(intf.name,args.delay), shell=True, stdout=subprocess.PIPE).stdout
            sleep(5)
            
            # - config
            config_folder = output_folder+"/R"+str(run)+current_folder+config_folder_name
            print (output_folder)
            ...
            # start voip clients
            start_voip_clients(serverHost, voip_host,  int(args.voipclients), subfolder, run)
            # start video clients
            print ("IP address something: " + ip_address_sh)
            start_video_clients(args.videoclients, "elastic", 30, ip_address_sh, net, subfolder, run, num_clients=total_num_hosts, output_folder=output_folder,current_folder=current_folder,config_folder=config_folder, dic=test_dict, cwd=cwd)
            #start_iperf(net)
            print ("sleeping")
            #throttleLink(bw_a)
            ...
            #sleep(15)
            genstats_voip_clients(serverHost, voip_host,  int(args.voipclients), subfolder, run)
            #sleep(5)
            #CLI(net)
